    ...
    <main className="grid min-h-screen place-items-center bg-slate-50 px-4 py-10">
      <div className="w-full max-w-md">
        <div className="mb-6 text-center">
          ...
          <h1 className="mt-2 text-2xl font-black text-slate-950">Fleet & Billing Platform</h1>
          <p className="mt-2 text-sm text-slate-500">Sign in to save bills securely and access history across devices.</p>
        </div>

        <Card>
          <CardHeader>
            <h2 className="text-base font-black text-slate-950">{isLogin ? "Login" : "Create Account"}</h2>
            <p className="mt-1 text-sm text-slate-500">{isLogin ? "Use your email and password to continue." : "Create a Supabase email/password account."}</p>
          </CardHeader>
          <CardContent>
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              {(authError || error) && (
                <div className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm font-semibold text-red-700">{error || authError}</div>
              )}
              {message && (
                <div className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-semibold text-[#1E3A8A]">{message}</div>
              )}
            ...
            <div className="mt-4 text-center text-sm text-slate-500">
              ...
              <button
                className="cursor-pointer font-bold text-[#1E3A8A] hover:underline"
                type="button"
                onClick={() => {
