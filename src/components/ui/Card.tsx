...
export function Card({ children, className, ...props }: PropsWithChildren<HTMLAttributes<HTMLDivElement>>) {
  return (
    <section className={cn("rounded-2xl border border-slate-200 bg-white shadow-soft", className)} {...props}>
      {children}
    </section>
  );
}
...
export function CardHeader({ children, className, ...props }: PropsWithChildren<HTMLAttributes<HTMLDivElement>>) {
  return <div className={cn("border-b border-slate-100 p-5", className)} {...props}>{children}</div>;
}
